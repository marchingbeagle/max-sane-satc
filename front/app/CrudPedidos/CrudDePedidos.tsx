...
const CrudDePedidos: React.FC = () => {
  ...
  const handleEditOrder = (orderId: string) => {
    ...
    if (order) {
      setFormData({
        ...order,
        data_pedido: moment(order.data_pedido, "DD/MM/YYYY").format(
          "YYYY-MM-DD"
        ),
      });
    }
  ...
  const handleSaveOrder = async () => {
    if (
      !Object.keys(formData.product_quantities).length ||
      !formData.user_id ||
      formData.quantidade < 1 ||
      !formData.data_pedido
    ) {
      alert(
        "Por favor, preencha todos os campos e certifique-se de que a quantidade é maior ou igual a 1."
      );
      return;
    }
    ...
    for (const [productId, quantity] of Object.entries(
      formData.product_quantities
    )) {
      const product = products.find((p) => p.id === productId);
      if (product && quantity > product.quantity_available) {
        errors[
          productId
        ] = `Quantidade disponível insuficiente. Disponível: ${product.quantity_available}`;
        hasErrors = true;
      }
    }
    ...
    const formattedDataPedido = moment(
      formData.data_pedido,
      "DD/MM/YYYY"
    ).format("YYYY-MM-DD");
    ...
    try {
      if (view === View.CREATE) {
        ...
        await axios.patch(`http://localhost:3001/pedidos/${selectedOrderId}`, {
          ...formData,
          data_pedido: formattedDataPedido,
        });
      }
  ...
  const calculateTotal = () => {
    return Object.entries(formData.product_quantities).reduce(
      (total, [productId, quantity]) => {
        const product = products.find((p) => p.id === productId);
        return (
          total +
          (product ? parseFloat(product.price.toString()) * quantity : 0)
        );
      },
      0
    );
  ...
  const calculateOrderTotal = (order: Order) => {
    return Object.entries(order.product_quantities).reduce(
      (total, [productId, quantity]) => {
        const product = products.find((p) => p.id === productId);
        return (
          total +
    ...
    <div className="p-6 m-6 bg-gray-100 rounded-lg shadow-lg">
      ...
      {view === View.LIST && (
        <div>
          ...
          <button
            className="bg-gray-800 text-white px-6 py-2 rounded-lg shadow hover:bg-gray-900 mb-4"
                ...
                >
                  <div>
                    ...
                    <p className="text-gray-600">
                      Usuário:{" "}
                      {users.find((u) => u.id === order.user_id)?.user_name}
                    </p>
      ...
      {(view === View.CREATE || view === View.EDIT) && (
        <div>
          <h1 className="text-3xl font-semibold mb-6">
            {view === View.CREATE ? "Novo Pedido" : "Editar Pedido"}
          </h1>
          <form
            onSubmit={(e) => {
              e.preventDefault();
          ...
          >
            <div>
              ...
              <div className="mt-4">
                ...
                <table className="w-full table-fixed border-collapse">
                  ...
                  <tbody>
                    {Object.entries(formData.product_quantities).map(
                      ([productId, quantity]) => {
            ...
            <div className="flex space-x-4">
              <button
                type="submit"
                className="bg-gray-800 text-white px-6 py-2 rounded-lg shadow hover:bg-gray-900"
              >
                {view === View.CREATE ? "Cadastrar" : "Salvar"}
              </button>
              <button
                className="bg-gray-300 text-gray-700 px-6 py-2 rounded-lg shadow hover:bg-gray-400"
      ...
      {view === View.DETAILS && selectedOrderId && (
        <div>
          ...
          <button
            className="bg-gray-300 text-gray-700 px-4 py-2 rounded-lg shadow hover:bg-gray-400 mb-4"
            onClick={() => setView(View.LIST)}
          >
            Voltar
          </button>
            ...
            {orders
              ...
              .map((order) => (
                <div key={order.id}>
                  ...
                  <p>
                    <strong>Data do Pedido:</strong> {order.data_pedido}
                  </p>
                  <p>
                    <strong>Produtos:</strong>
                  </p>
                  <ul className="list-disc ml-5">
                    {order.product_quantities &&
                      Object.entries(order.product_quantities).map(
                        ([productId, quantity]) => {
                          const product = products.find(
                            (p) => p.id === productId
                          );
                          return (
                            <li key={productId}>
                              {product?.name} - {quantity} unidades - R${" "}
                              {(product
                                ? parseFloat(product.price.toString()) *
                                  quantity
                                : 0
